...
model, in the original (intensional) Martin-L\"of type theory
...
\item It can be shown, by a very complicated and interesting argument,
 ...
 \item For a function $f:X\to Y$, consider the type
   ...
   \]
    ...
    $\Iso(f)$ rather than $\isEquiv(f)$, is provably empty. So, to
    have a consistent axiom, it is crucial to use the type
    ...
    elegant, but very difficult to arrive at. It is motivated by
    homotopical models of the theory. But the univalence axiom can be
    understood without reference to homotopy theory.
...
\item The fact (again proved by Voevodsky, with the model of simplicial
 ...
 \item Thus, we see that the formulation of univalence is far from
  ...
  Coquand and Mike Shulman on draft versions of these notes.
