...
% convertex to latex from lagda 2nd March 2018

\usepackage{amsmath,amssymb,a4,url}
...
The univalence axiom~\cite{hottbook,grayson} is not true or false
...
This is because
\begin{quote}
   univalence is a property of the \emph{identity type}
   of a universe of types.
\end{quote}
Nothing like Martin-L\"of's identity type occurs in ZFC or topos logic
as a \emph{native} concept, like in MLTT. Of course, we can create
\emph{models} of the identity type and univalence in these theories,
which will make univalence hold or fail. But in these notes we try to
understand the primitive concept of identity type, and the univalence
axiom, directly and independently of any such particular model, as in
the original (intensional) Martin-L\"of type theory
(1979)~\cite{MR682410}. In particular, we don't use the equality sign
``$=$'' to denote the identity type $\Id$, or think of it as a path
space.
...
is true or false (like a membership relation $x \in X$ in ZFC). For
example, if $\N$  is the type of natural numbers, we may write
\begin{gather*}  
    17 : \N, \\
   (13,17) : \N \times  \N. \\
\end{gather*}
...
\begin{eqnarray*}
    17 : \N \times  \N & \text{(nonsense)}, \\
   (13,17) : \N & \text{(nonsense)}.  
\end{eqnarray*}
...
product~$\times$ , and disjunction by the binary disjoint sum~$+$, and
...
It will be the case that for some other types $X$, even in the
presence of univalence, $\K(X)$ holds. Such types are called sets. The
$\K$ axiom says that all types are sets. The univalence axiom implies
that some types are not sets. (Then they will instead be 1-groupoids,
or 2-groupoids, ..., or even $\infty$-groupoids, with such notions
defined within MLTT rather than via models, but we will not address
this important aspect of univalent mathematics here).
...
to formulate of the univalence of $\U$:
\[
...
\item The fact (again proved by Voevodsky, with the model of simplicial
    ...
    such as $X:=\N$ and $Y:=\N\times \N$ by a property $P:\U\to \U$ such that $P(X)$ holds
    but $P(Y)$ doesn't.  This is because, being isomorphic, $X$ and $Y$ are
    equivalent. But then univalence implies $\Id(X,Y)$, which in turn
    implies $P(X) \iff P(Y)$ using $\J$.  Because univalence is consistent, it
    follows that for any given concrete $P:\U\to \U$, it is impossible to
