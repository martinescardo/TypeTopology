...
\begin{numbered} \df{(Injectivity in terms of algebraic injectivity.)}
...
with respect to a universe \m{\T} for truth-values is defined by
%
...
\M{
  ...
  \sigma (\mu (P , \phi , i)) & \eqdef & \text{from the fact that subsingletons are closed under sums.} \\
  \end{array}
}
%
\begin{numbered}
...
\noindent The functoriality and naturality conditions are even easier
to check, and we omit the verification. We now turn to algebras. We
omit the asy verification of the following.
...
\begin{numbered} Let \m{X:\U} be any type.
  ...
  \item The unit law for monad algebras amounts to, for any \m{x:X},
    ...
    \medskip Therefore a functor algebra satisfying the unit law amounts to the same thing as algebraic flabiness data. In other words, the algebraically \m{\T}-flabby types are the algebras of the pointed functor \m{(\Lift_\T,\eta)}. In particular, monad algebras are algebraically flabby.
  ...
  \end{enumerate}
\end{numbered}
\noindent So the associativity law for algebras plays no role in flabiness. But
of course we can have algebraic flabiness data that is associative,
such as not only the free algebra \m{\Lift_\T X}, but also the
following two examples that connect to the opening development of this
paper on the injectivity of universes.
...
\begin{theorem} (Characterization of injectivity in terms of algebraic injectivity.)
...
\end{corollary}
\begin{proof}
  The \m{D} algebraically \m{\T,\T}-injective in an unspecified way,
  and so by functoriality of truncation on objects and algebraic
  injective resizing, it is algebraically \m{\U,\V}-injective in an
  unspecified way, and hence it is \m{\U,\V}-injective.
\end{proof}
