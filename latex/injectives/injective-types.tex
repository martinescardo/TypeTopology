...
\usepackage{amsmath,stmaryrd,amssymb,amsthm,enumerate,bbm,url,hyperref,scalerel,mathtools}
...
The left and Kan extensions trivially satisfy \m{f \edown \id \sim f}
...
%     \begin{diagram}
% X &  \rTo^j &  Y & \rTo^k & Z \\
%   & \rdTo_f^{\precsim} & \dTo_{f\edown j} & \ldTo_{(f\edown j)\eup k \sim f\edown (k \comp j)}^{\precsim} \\
%   &         &  D,         &
% \end{diagram}
% \qquad
% \begin{diagram}
...
\end{numbered}
\begin{proof}
This is direct consequence of the laws
  %
  \M{
    \begin{array}{lll}
      (\Sigma (t : \Sigma (t : \Sigma B) , C \, t) \simeq (\Sigma (a : A)\,
    \Sigma (b : B \, a), C(a,b)) \\
      (\Pi (t : \Sigma (t : \Sigma B) , C \, t) \simeq (\Pi (a : A)\,
    \Pi (b : B \, a), C(a,b))
    \end{array}
  }
  %
...
\noindent The construction is routine, and presumably it can also be
