...
\noindent
And we have the same situation with injective types.
\begin{numbered}
  If excluded middle holds, then every inhabited type of any universe is
  injective with respect to any two universes.
\end{numbered}
\begin{proof}
  Because excluded middle gives algebraic injectivity, which gives
  injectivity.
\end{proof}
...
\begin{numbered}
  Under \m{\Omega}-resizing, if then every inhabited type in a universe \m{\U} is
...
\end{proof}
\noindent Putting the above together with some specializations, we get the following.
