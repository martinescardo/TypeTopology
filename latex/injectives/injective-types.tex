...
\usepackage{amsmath,stmaryrd,amssymb,amsthm,enumerate,bbm,url,scalerel,mathtools}
...
\newcommand{\ttrunc}[1]{\leftcolor{darkgreen}\llbracket\,#1\,\rightcolor{darkgreen}\rrbracket}
...
\begin{abstract}
  ...
  the main results are easy to state: (1)~Injectivity is equivalent to
  the propositional truncation of algebraic injectivity.  (2)~The
  algebraically injective types are precisely the retracts of
  ...
  injective \m{(n+1)}-types are precisely the retracts of universes of
  \m{n}-types. (3)~The algebraically injective types are also
  precisely the retracts of algebras of the partial-map
  ...
  resizing, we have similar results which have subtler statements and
  proofs that need to keep track of universe levels rather explicitly.
\end{abstract}
...
injective, if and only if all inhabited types are injective, so
that there is nothing interesting to say about injectivity in its presence.
...
Most constructions developed in this paper are in the absence of propositional
...
\item We work within an intensional Martin-L\"of type theory with types
  ...
  type \m{\N} of natural numbers, but we don't mention it as is it not
...
\item We often express a type of the form \m{\Sigma(x:X), A \, x} by
 ...
 remark that, in general, this is weaker than \m{f} being
...
steps we have we have ommitted here because we considered them to be
...
\section{Algebraic injectivity and flabbiness in the presence of
...
\begin{numbered} \df{(Characterization of algebraic injectives).}
...
\begin{numbered} \label{interms} \df{(Injectivity in terms of algebraic injectivity.)}
...
\section{Algebraic flabbiness and injectivity via the partial-map classifier}

The lifting of a type \m{X:\U} with respect to a universe \m{\T} for truth-values is defined by
%
\M{
\Lift_\T X \eqdef \Sigma (P : \T), (P \to X) \times \text{\m{P} is a subsingleton}.
}
%
The idea is that an element of \m{\Lift_\T X} is a partial element of
\m{X} with extent of definition~\m{P:\T}. We say that such an element
is defined if \m{P} holds.  This construction is well known in 1-topos
theory (see e.g.~\cite{}) and has been extended to univalent
mathematics in~\cite{}.  We have that \m{\Lift_\T} is the partial-map
classifier, in the sense that partial functions from \m{X} to \m{Y}
are in canonical bijection with functions \m{X \to \Lift_\T Y}, where
a partial function from \m{X} to \m{Y} is an ordinary function \m{A
  \to Y} together with an embedding \m{A \to X}.

This is a sort of an \m{\infty}-monad modulo universe
juggling~\cite{TypeTopology} and modulo providing coherence data,
which we haven't done (but we believe to be automatic), but the full
monad structure is not needed for our purposes.  The unit \m{\eta : X
  \to \Lift_\T X} is given by \M{\eta_X \, x = (\One, (p \mapsto x), i)}
where \m{i} is a proof that \m{\One} is a proposition. It is not hard
to see that
\begin{numbered}
  The unit \m{\eta_X : X\to\Lift_\T X} is an embedding.
\end{numbered}
Next, \m{\Lift_\T} is functorial by mapping a function
...
%
Any algebra of this would-be-monad is, in particular, a functor
algebra, meaning simply a map \m{\Lift_\T X \to X}. The following is
immediate:
\begin{numbered}
  A functor algebra \m{\Lift_\T X \to X} amounts to
  algebraic \m{\T}-flabby structure on~\m{X}.
\end{numbered}
\begin{numbered}
  We have \m{\mu_X : \Lift_\T(\Lift_\T X) \to \Lift_\T X} and so the
  type \m{\Lift_\T X} is \m{\T}-flabby and \m{\T,\T}-injective.
\end{numbered}
\begin{proof}
  Given a \m{P : \T}, a function \m{\Phi:P \to \Lift_\T X} and a proof \m{i} that P is a proposition, define
%
  \M{
    \mu_X(P , \Phi , i) = ...
...
that the upgraded assumption of \m{\Omega}-resizing does is to account
for the first universe too.

\begin{numbered}
  Under \m{\Omega}-resizing, for any type \m{X:\T}, the type
  \m{\Lift_\L X : \T^+} has an equivalent copy in the universe \m{\T}.
\end{numbered}

The following characterizes injectivity in terms of algebraic
injectivity.

\begin{theorem} In the presence of \m{\Omega}-resizing, the
  \m{\U,\U}-injectivity of a type \m{D} in a universe \m{\U} is
  equivalent to the propositional truncation of
  its algebraic \m{\U,\U}-injectivity.
\end{theorem}
\begin{proof}
 By \m{\Omega}-resizing, it is easy to see that the lifting of a type \m{D} as a copy in the same universe as \m{D}. Then we can use the same argument leading to~\ref{interms}.
\end{proof}
...
\begin{theorem}
  ...
  \m{D} in a universe \m{\U} is \m{\U,\U}-injective , then it is also
  \m{\V,\W}-injective for any universes \m{\V} and \m{\W}.
\end{theorem}
...
else tell that this is impossible. If \m{X} is a subsingleton, then,
under functional and propositional extensionality, the decidability of
\m{X} is equivalent to the proposition \M{(X = \One) + (X = \Zero).}
...
%
\M{
  \EM_\U \eqdef \Pi (P : \U), \text{\m{X} is a subsingleton \m{\to X + (X \to \Zero)}.}
}
%
As discussed in the introduction, we are not
assuming or rejecting this principle, which is independent from the
other axioms.  We remark that it is easy to prove that, under functional
...
For the converse, we consider, given a subsingleton \m{P}, the type \m{P + ¬ P + \One},
whose algebraic flabbiness gives the decidability of P.

\begin{lemma}
  For any subsingleton \m{P} in a universe \m{\W}, if the type \m{P +
    (P \to \Zero)} is algebraically \m{\W}-flabby, then \m{P} is
  decidable.
\end{lemma}
...
\begin{numbered}
  For any universe \m{\U}, if every pointed type in the universe
  \m{\W} is algebraically \m{\W,\U}-injective, then excluded middle
  holds in \m{\W}.
\end{numbered}
