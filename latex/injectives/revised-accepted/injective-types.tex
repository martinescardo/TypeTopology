% I apologize that msc.cls is not working for me under the standard
% texlive tex distribution in my operating system.

\documentclass[11pt]{article}

% Please don't change the punctuation, in particular commas. The
% author has thought very carefully about which commas he wishes to
% have or not. Please also allow me to write "don't" rather than "do
% not", etc. Thank you.

\usepackage[hidelinks]{hyperref}
\usepackage{amssymb,amsthm,enumerate}
\usepackage{amsmath,stmaryrd,url,scalerel,mathtools}

\usepackage[bb=boondox]{mathalfa} % for getting \mathbb{0} to work

% We use the diagrams package http://paultaylor.eu/diagrams/:
\usepackage[nohug]{diagrams}
% These are types of arrows for diagrams:
\newarrow{Into}{C}{-}{-}{-}{>}
\newarrow{Onto}{-}{-}{-}{-}{>>}
\newarrow{Eto}{.}{.}{.}{.}{>}
\newarrow{Id}{=}{=}{=}{=}{=}
...
% These things are defined in the MSCS style file (which unfortunately
% doesn't work for me) using the texlive distribution of tex:
\swapnumbers
...
% We want definitions not to be in italics:
\theoremstyle{definition}
\newtheorem{definition}[theorem]{Definition}


\title{Injective types in univalent mathematics}
...
\begin{abstract}
  ...
  % Don't know where to put the mathematics subject classifications in
  % the msc.cls style, so I leave them here for the moment:
  \medskip \noindent {\bf MSC 2010.} 03B15, 03B35, 03G30, 18A40, 18C15.

  % To go to the keyword environment of the msc.cls styl:
  \medskip \noindent {\bf keywords.} Injective type; flabby type; Kan
  extension; partial-map classifier; univalent mathematics; univalence
  axiom.
\end{abstract}
...
book~\cite{hottbook}, and Coq~\cite{coq}, in that we don't assume % please don't change "don't" to "do not" etc.
...
\item We stipulate that we have copies \m{\Zero_\U} and \m{\One_\U} of the
...
It is also easy to see that, without assuming \m{j} to be an embedding,
  ...
  laborious:
\begin{remark} For any \m{g : Y \to \T}, we have canonical equivalences
    \begin{enumerate}
...
\begin{remark} \label{iterated}
  For types \m{X : \U}, \m{Y : \V} and \m{Z : \W}, and functions \m{j : X \to Y}, \m{k : Y \to Z} and \m{f : X \to \U \sqcup \V \sqcup \W}, we have canonical identifications
%
  \M{
    \begin{array}{lll}
...
considering the case when \m{f} is the unique map \m{\Zero \to D}.
...
\end{lemma}
\begin{proof}
  Given a subsingleton \m{P:\U} and a map \m{f : P \to D}, we can take its extension \m{f \mid \operatorname{!}: \One \to D} along the unique map \m{!:P \to \One}, because it is an embedding, and then we let \m{d \eqdef (f \mid \operatorname{!})\, \one}, and the extension property gives \m{d = f \, p} for any \m{p:P}.
\end{proof}
...
\end{lemma}
\begin{proof}
  Given an embedding \m{j : X \to Y} of types \m{X:\U} and \m{Y:\V}, a map \m{f : X \to D} and a
  point \m{y:Y}, in order to construct \m{(f \mid j) \, y} we consider
  the map \m{f_y : j^{-1}(y) \to D} defined by \m{(x,p) \mapsto
    f\,x}. Because the fiber \m{j^{-1}(y) : \U \sqcup \V} is a subsingleton as \m{j} is an
...
\m{\Pi}. (iii) Any universe \m{\U} seen as a subuniverse of \m{\U
  \sqcup \V}.
...
\end{lemma}
\begin{proof}
  We first get the \m{\U}-flabbiness of \m{D} by~\ref{for27:1}, and then its
  \m{\U' \sqcup \V'}-flabbiness by~\ref{universe:independence}, and finally its algebraic
  \m{\U',\V'}-injectivity by~\ref{for27:2}.
\end{proof}
...
\end{theorem}
\begin{proof}
  The universe \m{\U} is algebraically \m{\U,\U}-injective
  by~\ref{ref:16:1}, and hence it is algebraically \m{\U^+,(\U \sqcup
    \V)^+}-injective by~\ref{universe:independence}, which has the
  ...
  a larger type, in this case the embedding of the universe \m{\U}
  into the larger universe \m{\U \sqcup \V} constructed
  in~\ref{canonical}.
\end{proof}
...
\begin{lemma} Let \m{X:\U} be any type.
  \begin{enumerate}
...
\begin{lemma} The universe \m{\T} is a monad algebra of
...
\begin{theorem} % \df{(Logical equivalence of algebraic injectivity of all pointed types with excluded middle.)} \\
