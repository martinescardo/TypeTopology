...
has-size-idempotent ua 𝓤 𝓥 Y i (H , e) = X , γ
 where
  ...
  γ = X  ≃⟨ Σ-change-of-variable pr₁ (eqtofun e) (eqtofun-is-an-equiv e) ⟩
      X' ≃⟨ ϕ ⟩
      Y  ■
   where
    X' : 𝓥 ⁺ ⊔ 𝓤 ̇
...
Added 23 December 2020. The cumulativity of all A implies
propositional resizing:

\begin{code}

resizing-from-global-cumulativity⁺ :

   ((A : {𝓤 : Universe} → 𝓤 ̇ → 𝓤 ⁺ ̇) → cumulative⁺ A)
 → {𝓤 : Universe} → (Ω 𝓤) has-size 𝓤₁

resizing-from-global-cumulativity⁺ c {𝓤} = γ
 where
...
TODO. Can we get some form of resizing from global properties as follows?
...
resizing-from-global-cumulativity c = γ
 where
  A : {𝓤 : Universe} → 𝓤 ̇ → 𝓤 ̇
  A {𝓤} _ = {!!}
