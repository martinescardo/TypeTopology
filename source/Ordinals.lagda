...
transfinite-recursion : well-founded → {Y : Set} → 
  ((x : X) → ((y : X) → y < x → Y) → Y) → X → Y
transfinite-recursion w {Y} = transfinite-induction w (λ x → Y)
...
is-accessible-is-prop fe fe' = is-accessible-induction P lemma
 where
  P : (x : X) → is-accessible x → U ⊔ V ̇
  P x a = (b : is-accessible x) → a ≡ b

  lemma : (x : X) (σ : (y : X) → y < x → is-accessible y) →
      ((y : X) (l : y < x) (a : is-accessible y) → σ y l ≡ a) →
      (b : is-accessible x) → next x σ ≡ b
  lemma x σ IH b = next x σ ≡⟨ ap (λ f → next x f) (dfunext fe (λ y → dfunext fe' (h y))) ⟩
...
antisym = transfinite-induction' P (λ x f y l m → f y l x m l) 
 where
  P : X → U ⊔ V ̇
  P x = (y : X) → y < x → y ≤ x
...
notricycle : (x : X) → is-accessible x
           → (y z : X) → y < x → z < y → z ≤ x
notricycle = transfinite-induction' P (λ x f y z l m n → f y l z x m n l) 
 where
  P : X → U ⊔ V ̇
