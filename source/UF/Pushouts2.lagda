...
module UF.Pushouts2 (fe : Fun-Ext) where
...
begin{code}
 ...
 pushout-dependent-universal-property
  : {P : pushout → 𝓣  ̇}
  → Pushout-Dependent-Universal-Property pushout f g pushout-cocone P
 pushout-dependent-universal-property {_} {P} = {!!}
  where
   ϕ : ((x : pushout) → P x) → dependent-cocone f g pushout pushout-cocone P
   ϕ d = canonical-map-to-dependent-cocone pushout f g (inll , inrr , glue) P d
   ψ : dependent-cocone f g pushout pushout-cocone P → ((x : pushout) → P x)
   ψ (i , j , H) = pushout-induction i j H
   ψ-ϕ : ψ ∘ ϕ ∼ id
   ψ-ϕ d = dfunext fe {!!}
   ϕ-ψ : ϕ ∘ ψ ∼ id
   ϕ-ψ (i , j , H)
    = inverse-dependent-cocone-map f g pushout pushout-cocone P
       (ϕ (ψ (i , j , H))) (i , j , H)
       (pushout-induction-comp-inll i j H ,
        pushout-induction-comp-inrr i j H ,
        ∼-sym {!pushout-induction-comp-glue!})
