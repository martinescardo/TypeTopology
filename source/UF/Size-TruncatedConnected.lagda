...
\begin{code}
 ...
 Lemma-2-3[being-truncated-gives-local-smallness] : {X : 𝓤 ̇} {n : ℕ₋₂}
                                                  → Propositional-Resizing
                                                  → X is (n + 1) truncated
                                                  → X is ι (n + 2) locally-small
 Lemma-2-3[being-truncated-gives-local-smallness] {_} {X} {−2} pr X-prop =
  pr X (is-prop'-implies-is-prop X-prop)
 Lemma-2-3[being-truncated-gives-local-smallness] {_} {_} {succ n} pr X-trunc x x' =
  Lemma-2-3[being-truncated-gives-local-smallness] pr (X-trunc x x')

 being-truncated-gives-local-smallness =
  Lemma-2-3[being-truncated-gives-local-smallness]

\end{code}

We note that Lemma-2-3 provides is one side of a bimplication involving
propositional resizing.We will now record the other direction.

\begin{code}

 being-truncated-gives-local-smallness-implies-resizing
 ...
 Lemma-2-4[type-with-truncated-map-to-locally-small-type-is-locally-small]
  ...
  local-smallness-is-closed-under-≃ (total-fiber-is-domain f)
   (local-smallness-is-closed-under-Σ Y-loc-small
    (λ y → Lemma-2-3[being-truncated-gives-local-smallness] pr (f-trunc y)))
...
In [1] Christense proves Theorem 2.6. with propositional resizing but later found
a proof that avoids it. Since we are interested in avoiding unneccesary use of
resizing we will simply record the second proof. But first we need a few lemmas.
...
\begin{code}
 ...
 set-truncation-of-universe-is-large : Join-Construction-Result 
                                     → is-large ∥ 𝓥 ̇ ∥[ 0 ]
 set-truncation-of-universe-is-large j =
  contrapositive small-from-small-set-trunc universes-are-large
  where
   small-from-small-set-trunc : is-small ∥ 𝓥 ̇ ∥[ succ −1 ] → is-small (𝓥 ̇)
   small-from-small-set-trunc small-trunc =
