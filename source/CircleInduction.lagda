...
-}
   ...
   test3 : pr₁ (from-Σ-≡ lemma₄) ⁻¹ ∙ ap ρ loop ∙ pr₁ (from-Σ-≡ lemma₄) ≡ l⁺
   test3 = (transport-along-≡-dup (pr₁ (from-Σ-≡ lemma₄)) (ap ρ loop)) ⁻¹ ∙ pr₂ (from-Σ-≡ lemma₄)

   test4 : ap pr₁ lemma₄ ⁻¹ ∙ ap ρ loop ∙ ap pr₁ lemma₄ ≡ l⁺
   test4 = (transport-along-≡-dup (ap pr₁ lemma₄) (ap ρ loop)) ⁻¹ ∙ from-Σ-≡' lemma₄


--    blah : pr₁ (ρ base) ≡ pr₁ (ρ base)
--    blah = ap pr₁ (ap ρ loop)

--    blah-is-refl : blah ≡ refl
--    blah-is-refl = {!!}

--    blahblah : transport (A ∘ pr₁) (ap ρ loop) (𝕊¹-induction base) ≡ 𝕊¹-induction base
--    blahblah = apd pr₂ (ap ρ loop)

--    blah₃ : transport A loop (𝕊¹-induction base) ≡ 𝕊¹-induction base
--    blah₃ = transport A loop (𝕊¹-induction base) ≡⟨ refl ⟩
--            transport (A ∘ pr₁ ∘ ρ) loop (𝕊¹-induction base) ≡⟨ transport-ap (A ∘ pr₁) ρ loop ⟩
--            transport (A ∘ pr₁) (ap ρ loop) (𝕊¹-induction base) ≡⟨ blahblah ⟩
--            𝕊¹-induction base                                   ∎

--    fff : transport A (ap pr₁ (ap ρ loop)) (pr₂ (ρ base)) ≡
--            pr₂ (ρ base)
--    fff = {!!}

--    foo : apd 𝕊¹-induction loop ≡ blah₃
--    foo = {!!}

--    qwerty : {!!}
--    qwerty = {!!}

--    hhh : transport (λ - → - ≡ -) (ap pr₁ lemma₄) (ap ρ loop) ≡ l⁺
--    hhh = {!!} -- apd pr₂ lemma₄
