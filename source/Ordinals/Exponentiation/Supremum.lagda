...
exp-+-distributes {𝓤} {𝓥} α β =
 transfinite-induction-on-OO (λ γ → α ^ₒ (β +ₒ γ) ＝ α ^ₒ β ×ₒ α ^ₒ γ) {!!}
  where
   I : (γ : Ordinal 𝓥)
     → ((c : ⟨ γ ⟩) → α ^ₒ (β +ₒ (γ ↓ c)) ＝ α ^ₒ β ×ₒ α ^ₒ (γ ↓ c))
     → α ^ₒ β ×ₒ α ^ₒ γ
       ＝ sup (cases (λ (_ : 𝟙 {𝓤}) → α ^ₒ β)
                     (λ (c : ⟨ γ ⟩) → α ^ₒ (β +ₒ (γ ↓ c)) ×ₒ α))
   I γ IH = α ^ₒ β ×ₒ α ^ₒ γ ＝⟨ ap (α ^ₒ β ×ₒ_) (^ₒ-behaviour α γ) ⟩
            α ^ₒ β ×ₒ (sup (cases (λ _ → 𝟙ₒ) (λ c → α ^ₒ (γ ↓ c) ×ₒ α))) ＝⟨ ×ₒ-preserves-suprema pt sr (α ^ₒ β) (cases (λ _ → 𝟙ₒ) (λ c → α ^ₒ (γ ↓ c) ×ₒ α)) ⟩
...
-}
  ...
  {-
   I : (γ : Ordinal 𝓥)
     → ((c : ⟨ γ ⟩) → α ^ₒ (β +ₒ (γ ↓ c)) ＝ α ^ₒ β ×ₒ α ^ₒ (γ ↓ c))
     → α ^ₒ (β +ₒ γ) ＝ α ^ₒ β ×ₒ α ^ₒ γ
   I γ IH = ⊴-antisym (α ^ₒ (β +ₒ γ)) (α ^ₒ β ×ₒ α ^ₒ γ) II III
    where
     II : α ^ₒ (β +ₒ γ) ⊴ α ^ₒ β ×ₒ α ^ₒ γ
     II = ^ₒ-is-lower-bound-of-upper-bounds α (β +ₒ γ) (α ^ₒ β ×ₒ α ^ₒ γ)
           (⊴-trans 𝟙ₒ (α ^ₒ β) (α ^ₒ β ×ₒ α ^ₒ γ)
             (^ₒ-has-least-element α β)
             (^ₒ-×ₒ-right-⊴ α β (α ^ₒ γ) (^ₒ-has-least-element α γ)))
           l
      where
       l : (x : ⟨ β +ₒ γ ⟩) → α ^ₒ (β +ₒ γ ↓ x) ×ₒ α ⊴ α ^ₒ β ×ₒ α ^ₒ γ
       l (inl b) = ⊴-trans
                    (α ^ₒ (β +ₒ γ ↓ inl b) ×ₒ α)
                    (α ^ₒ β)
                    (α ^ₒ β ×ₒ α ^ₒ γ)
       ...
       III₁ = sup-is-lower-bound-of-upper-bounds _ _ ub
        where
         ub : (i : 𝟙 + ⟨ β +ₒ γ ⟩)
            ...
            ⊴ sup (cases (λ _ → α ^ₒ β) (λ c → α ^ₒ (β +ₒ (γ ↓ c)) ×ₒ α))
         ub (inl ⋆) = ⊴-trans 𝟙ₒ (α ^ₒ β) _ (^ₒ-has-least-element α β) (sup-is-upper-bound _ (inl ⋆))
         ub (inr (inl b)) = ⊴-trans _ (α ^ₒ β) _
                             (transport⁻¹ (_⊴ α ^ₒ β) (ap (λ - → α ^ₒ - ×ₒ α) ((+ₒ-↓-left b) ⁻¹)) (^ₒ-is-upper-bound₂ α β))
