...
Note: Tom de Jong's construction of suprema (in Ordinals.OrdinalOfOrdinalsSuprema)
requires set-quotients. The current construction of Infima does not require
these; apart from univalence, only propositional truncation is required to
express that I is inhabited.

Christian Sattler pointed out to me that there is also another simple construction:
The infimum is the supremum of all ordinals that are below everything in the family.

NOTE: There is also a different notion of Infimum by Escardo in the file
Ordinals.InfProperty.lagda from 2012.
...
Some basic lemmas that help proving that a type with relation is an ordinal.
...
module inf-construction
 ...
 is-compatible-is-prop : (f : (i : I) → ⟨ F i ⟩) → is-prop (is-compatible f)
 is-compatible-is-prop f =
...
For every i : I the "projection" c : Inf → F i is monotone, an embedding, and
satisfies the simulation property. (None of this requires I to be inhabited.)
...
\begin{code}
 ...
 module _ (i : I) where
  ...
  c-lc {f , p} {g , q} e =
   to-subtype-＝ is-compatible-is-prop (dfunext fe' ptwise)
    where
     ptwise : (j : I) → f j ＝ g j
...
Much further below, we show that the un-negated conclusion is a constructive taboo.
...
The "dual" or the standard result for suprema:
...
module greatest-common-initial-segment
 ...
 is-common-lower-segment-is-prop : (x : ⟨ F i₀ ⟩)
                                 → is-prop (is-common-lower-segment x)
 is-common-lower-segment-is-prop x = Π-is-prop fe' γ
  where
 ...
 is-common-lower-segment-is-lower-closed :
    (x : ⟨ F i₀ ⟩) → is-common-lower-segment x
  → (z : ⟨ F i₀ ⟩) → z ≺⟨ F i₀ ⟩ x → is-common-lower-segment z
 is-common-lower-segment-is-lower-closed x cx z l j =
 ...
 ≪-is-extensional (x , cx) (y , cy) u v =
  to-subtype-＝ is-common-lower-segment-is-prop (Extensionality (F i₀) x y u' v')
   where
    u' : (z : ⟨ F i₀ ⟩) → z ≺⟨ F i₀ ⟩ x → z ≺⟨ F i₀ ⟩ y
    u' z l = u (z , is-common-lower-segment-is-lower-closed x cx z l) l
    v' : (z : ⟨ F i₀ ⟩) → z ≺⟨ F i₀ ⟩ y → z ≺⟨ F i₀ ⟩ x
    v' z l = v (z , is-common-lower-segment-is-lower-closed y cy z l) l
...
simulation; the map that reads off, for each j, the point realising the common
segment is a section of it. That makes it an order-isomorphism, so gcis is the
infimum.
...
\begin{code}
 ...
 to-gcis-is-initial-segment (f , p) (y , cy) l =
  pr₁ σ ,
  pr₁ (pr₂ σ) ,
  to-subtype-＝ is-common-lower-segment-is-prop (pr₂ (pr₂ σ))
   where
 ...
 to-gcis-from-gcis (x , cx) =
  to-subtype-＝ is-common-lower-segment-is-prop
...
Another simple observation: infima commute with imfima. An infimum over a Σ-type is
