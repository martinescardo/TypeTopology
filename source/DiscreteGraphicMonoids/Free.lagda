...
\begin{code}
 ...
 ϕ-is-hom (x • us) vs =
  ϕ (x • us ◦ vs)   ＝⟨ refl ⟩
  x ● ϕ (us ◦ vs)   ＝⟨ ap (x ●_) (ϕ-is-hom us vs) ⟩
  x ● (ϕ us ● ϕ vs) ＝⟨ ●-assoc x (ϕ us) (ϕ vs) ⟩
  (x ● ϕ us) ● ϕ vs ＝⟨ refl ⟩
  ϕ (x • us) ● ϕ vs ∎

 fold-map-lemma : (xs : List X) → f' xs ＝ ϕ (map f xs)
 fold-map-lemma [] = refl
 fold-map-lemma (x • xs) = ap (f x ●_) (fold-map-lemma xs)
 ...
 ϕ-δ-lemma u v (w • ws) = h (M-is-discrete u w)
  where
   ...
   h (inl refl) =
    ...
    (u ● v) ● ϕ ws             ＝⟨ ap (_● ϕ ws) ((graphic u v)⁻¹) ⟩
 ...
 fold-ρ-lemma : (us vs : List M)
              → ρ us ＝ ρ vs → ϕ us ＝ ϕ vs
 fold-ρ-lemma = course-of-values-induction-on-length _ h
  where
   h : (us : List M)
     → ((us' : List M)
             → length us' < length us
 ...
 fold-ρ : (us : List M) → ϕ us ＝ ϕ (ρ us)
 fold-ρ us = fold-ρ-lemma us (ρ us) ((ρ-idemp us)⁻¹)

 ρ-lemma : (xs : List X)
          → f' (ρ xs) ＝ f' xs
 ρ-lemma xs =
  f' (ρ xs)            ＝⟨ fold-map-lemma (ρ xs) ⟩
  ϕ (map f (ρ xs))     ＝⟨ fold-ρ (map f (ρ xs)) ⟩
  ϕ (ρ (map f (ρ xs))) ＝⟨ ap (ϕ) (ρ-map f xs) ⟩
  ϕ (ρ (map f xs))     ＝⟨ (fold-ρ (map f xs))⁻¹ ⟩
  ϕ (map f xs)         ＝⟨ (fold-map-lemma xs)⁻¹ ⟩
  f' xs                ∎

 ρ-lemma' : (xs ys : List X)
          → ρ xs ＝ ρ ys
          → f' xs ＝ f' ys
 ρ-lemma' xs ys e =
  f' xs     ＝⟨ (ρ-lemma xs)⁻¹ ⟩
