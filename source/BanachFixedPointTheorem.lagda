...
A version of the Banach fixed-point theorem for co-ultrametric types.
...
First we define a 'co-ultrametric type', which is a type equipped with
a cometric, but rather than triangle equality we have the ultrametric 
property. 

 A co-ultrametric type is a type X,
...
In the code, we refer to the underlying type of a co-ultrametric
type (CUT) C as ⟨ C ⟩.
A CUT C is nonempty simply if we can exhibit a single element of ⟨ C ⟩.

\begin{code}

CoUltrametricType : 𝓤 ⁺ ̇
CoUltrametricType {𝓤} = Σ X ꞉ 𝓤 ̇ , Σ c ꞉ (X → X → ℕ∞) , is-codistance c

⟨_⟩ CUT-Nonempty : CoUltrametricType → 𝓤 ̇
⟨ X , _ ⟩ = X
CUT-Nonempty = ⟨_⟩
...
CUT-ContractionMapping : CoUltrametricType → 𝓤 ̇
...
BanachFixedPointTheorem : (C : CoUltrametricType {𝓤})
                        → CUT-Nonempty C 
                        → CUT-Complete C 
