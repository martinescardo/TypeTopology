...
module C-Space.UsingFunExt.ComputationExperiments where
...
open import C-Space.UsingFunExt.UCinT fe
...
C-Space.UniformContinuity.LM _＝_
UF.DiscreteAndSeparated.ℕ-is-discrete (λ α → 𝟚-induction 0 0 (α 0))
(1 Naturals.Addition.+
 C-Space.Preliminaries.Naturals.Order.max
 (pr₁
  (transport
   (λ x →
      Σ
      (λ n →
         ...
         (λ x₁ →
            ...
            n C-Space.Preliminaries.Naturals.Order.≤ n')))
   ...
   (transport
    ...
    (transport
     ...
     (λ p pP t uc q qA t₁ tC →
        ...
        (pr₁ (tC (pr₁ pP)))
        ,
        C-Space.UniformContinuity.Lemma[LM-modulus] _＝_
        ...
        ,
        C-Space.UniformContinuity.Lemma[LM-least] _＝_
        UF.DiscreteAndSeparated.ℕ-is-discrete
        (λ p₁ → transport (_＝ z) p₁ refl)
        (λ p₁ q₁ → transport (_＝_ z) q₁ p₁) (λ x → p (t₁ x))
        (pr₁ (tC (pr₁ pP)))
     ...
     (0 ,
      (λ α β e → refl) ,
      (λ k prk → C-Space.Preliminaries.Naturals.Order._≤_.≤-zero))
     (λ α x → α x)
     (λ k →
        C-Space.UniformContinuity.LM (λ α → _＝⟦_⟧_ α k)
        Lemma[＝⟦⟧-decidable] (λ α x → α x)
        (C-Space.UniformContinuity.LM (λ α → _＝⟦_⟧_ α (succ k))
         (λ α β →
            dep-cases
            (λ em →
         ...
         (λ x → cons (₀ ∷ ⟨⟩) x) k)
        ,
        C-Space.UniformContinuity.Lemma[LM-modulus] (λ α → _＝⟦_⟧_ α k)
        ...
        (λ α β el →
           ...
           (λ i i<k →
              ...
              (Lemma[＝⟦⟧-<]
               (Lemma[＝⟦⟧-succ]
                (C-Space.UniformContinuity.Lemma[LM-modulus]
                 (λ α₁ → _＝⟦_⟧_ α₁ (succ k))
                 (λ α₁ β₁ →
                    dep-cases
                    (λ em →
                 ...
                 (λ α₁ β₁ e →
                    ...
                    (Lemma[<-＝⟦⟧]
                     (λ i₁ r →
                        C-Space.UniformContinuity.claim₀ ₀ ⟨⟩ k α₁ β₁ e e i₁
                        (C-Space.Preliminaries.Naturals.Order.≤-trans r
                         (C-Space.Preliminaries.Naturals.Order.Lemma[n≤n+1] k))))
                    (C-Space.UniformContinuity.claim₀ ₀ ⟨⟩ k α₁ β₁ e e k
                     (C-Space.Preliminaries.Naturals.Order._≤_.≤-succ
                      C-Space.Preliminaries.Naturals.Order.≤-refl)))
                 α β el))
               i i<k)
              refl))
        ,
        C-Space.UniformContinuity.Lemma[LM-least] (λ α → _＝⟦_⟧_ α k)
        Lemma[＝⟦⟧-decidable] ＝⟦⟧-sym ＝⟦⟧-trans (λ α x → α x)
        (C-Space.UniformContinuity.LM (λ α → _＝⟦_⟧_ α (succ k))
         (λ α β →
            dep-cases
