...
transport₂ : {X : U ̇} {Y : V ̇} (A : X → Y → W ̇)
             {x x' : X} {y y' : Y}
          → x ≡ x' → y ≡ y' → A x y → A x' y'
transport₂ A refl refl = id

back-transport₂ : {X : U ̇} {Y : V ̇} (A : X → Y → W ̇)
                  {x x' : X} {y y' : Y}
               → x ≡ x' → y ≡ y' → A x' y' → A x y
...
transport-prop : {X : U ̇} {Y : X → V ̇} (P : {x : X} → Y x → W ̇)
               (x : X) (y : Y x) → P y → (x' : X) (r : x ≡ x') → P(transport Y r y)
transport-prop P x y p .x refl = p

transport-rel : {X : U ̇} {Y : X → V ̇} (_≺_ : {x : X} → Y x → Y x → W ̇)
...
transport-const : {X : U ̇} {Y : V ̇} {x x' : X} {y : Y} (p : x ≡ x')
               → transport (λ (_ : X) → Y) p y ≡ y
transport-const refl = refl

apd' : {X : U ̇} (A : X → V ̇) (f : (x : X) → A x) {x y : X}
...
ap-id-is-id : {X : U ̇} {x y : X} (p : x ≡ y) → p ≡ ap id p
ap-id-is-id refl = refl

ap-comp : {X : U ̇} {Y : V ̇} (f : X → Y) {x y z : X} (p : x ≡ y) (q : y ≡ z)
       → ap f (p ∙ q) ≡ ap f p ∙ ap f q
ap-comp f refl refl = refl

ap-sym : {X : U ̇} {Y : V ̇} (f : X → Y) {x y : X} (p : x ≡ y)
       → (ap f p) ⁻¹ ≡ ap f (p ⁻¹)
ap-sym f refl = refl

ap-ap : {X : U ̇} {Y : V ̇} {Z : W ̇} (f : X → Y) (g : Y → Z) {x x' : X} (r : x ≡ x')
...
ap₂ : {X : U ̇} {Y : V ̇} {Z : W ̇} (f : X → Y → Z) {x₀ x₁ : X} {y₀ y₁ : Y}
   → x₀ ≡ x₁ → y₀ ≡ y₁ → f x₀ y₀ ≡ f x₁ y₁
ap₂ f refl refl = refl

refl-left-neutral : {X : U ̇} {x y : X} {p : x ≡ y} → refl ∙ p ≡ p
refl-left-neutral {U} {X} {x} {_} {refl} = refl

refl-right-neutral : {X : U ̇} {x y : X} (p : x ≡ y) → p ≡ p ∙ refl
refl-right-neutral p = refl

assoc : {X : U ̇} {x y z t : X} (p : x ≡ y) (q : y ≡ z) (r : z ≡ t)
      → (p ∙ q) ∙ r ≡ p ∙ (q ∙ r)
assoc refl refl refl = refl

happly' : {X : U ̇} {A : X → V ̇} (f g : Π A) → f ≡ g → f ∼ g
happly' f g p x = ap (λ - → - x) p

happly : {X : U ̇} {A : X → V ̇} {f g : Π A} → f ≡ g → f ∼ g
happly = happly' _ _

sym-is-inverse : {X : U ̇} {x y : X} (p : x ≡ y)
               → refl ≡ p ⁻¹ ∙ p
...
⁻¹-involutive : {X : U ̇} {x y : X} (p : x ≡ y) → (p ⁻¹)⁻¹ ≡ p
⁻¹-involutive refl = refl

⁻¹-contravariant : {X : U ̇} {x y : X} (p : x ≡ y) {z : X} (q : y ≡ z)
                → q ⁻¹ ∙ p ⁻¹ ≡ (p ∙ q)⁻¹
⁻¹-contravariant refl refl = refl

left-inverse : {X : U ̇} {x y : X} (p : x ≡ y) → p ⁻¹ ∙ p ≡ refl
left-inverse {U} {X} {x} {y} refl = refl

right-inverse : {X : U ̇} {x y : X} (p : x ≡ y) → refl ≡ p ∙ p ⁻¹
right-inverse {U} {X} {x} {y} refl = refl

cancel-left : {X : U ̇} {x y z : X} {p : x ≡ y} {q r : y ≡ z}
            → p ∙ q ≡ p ∙ r → q ≡ r
...
homotopies-are-natural' : {X : U ̇} {A : V ̇} (f g : X → A) (H : f ∼ g) {x y : X} {p : x ≡ y}
...
from-Σ-≡' : {X : U ̇} {Y : X → V ̇} {u v : Σ Y} (r : u ≡ v)
...
to-Σ-≡ : {X : U ̇} {A : X → V ̇} {σ τ : Σ A}
          → (Σ \(p : pr₁ σ ≡ pr₁ τ) → transport A p (pr₂ σ) ≡ pr₂ τ)
          → σ ≡ τ
to-Σ-≡ (refl , refl) = refl

ap-pr₁-to-Σ-≡ : {X : U ̇} {A : X → V ̇} {σ τ : Σ A}
                (w : Σ \(p : pr₁ σ ≡ pr₁ τ) → transport A p (pr₂ σ) ≡ pr₂ τ)
              → ap pr₁ (to-Σ-≡ w) ≡ pr₁ w
ap-pr₁-to-Σ-≡ (refl , refl) = refl

to-Σ-≡' : {X : U ̇} {Y : X → V ̇} {x : X} {y y' : Y x}
        → y ≡ y' → _≡_ {_} {Σ Y} (x , y) (x , y')
to-Σ-≡' {U} {V} {X} {Y} {x} = ap (λ - → (x , -))

fromto-Σ-≡ : {X : U ̇} {A : X → V ̇} {σ τ : Σ A} (w : Σ \(p : pr₁ σ ≡ pr₁ τ) → transport A p (pr₂ σ) ≡ pr₂ τ)
           → from-Σ-≡ (to-Σ-≡ w) ≡ w
fromto-Σ-≡ (refl , refl) = refl

tofrom-Σ-≡ : {X : U ̇} {A : X → V ̇} {σ τ : Σ A} (r : σ ≡ τ)
           → to-Σ-≡ (from-Σ-≡ r) ≡ r
tofrom-Σ-≡ refl = refl
