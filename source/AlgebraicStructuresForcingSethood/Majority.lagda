...
\begin{code}
 ...
 side₁-is-p : (p : ΩM) → ＝-cong idem₁ idem₁ (m' p refl₀ p) ＝ p
 side₁-is-p p = eq₁' p refl
  where
   eq₁' : {a b c d : M}
        → (p : a ＝ b)
        → (q : c ＝ d)
        → ＝-cong (eq₁ a c) (eq₁ b d) (m' p q p) ＝ p
   eq₁' {a} {_} {c} {_} refl refl = ＝-cong-refl (eq₁ a c)
...
\begin{code}
 ...
 side₀-is-refl p = use-eq₀ ∙ (＝-cong-refl idem₀)
  where
   idem₀ : m₀ ＝ m m₀ m₀ m₀
   idem₀ = (eq₀ m₀ m₀) ⁻¹
   ...
   eq₀' : {a b c d : M}
        → (p : a ＝ b)
        → (q : c ＝ d)
        → (m' q p p) ＝ ＝-cong ((eq₀ a c) ⁻¹) ((eq₀ b d) ⁻¹) p
   eq₀' {a} {_} {c} {_} refl refl = (＝-cong-refl ((eq₀ a c) ⁻¹)) ⁻¹

   use-eq₀ : (m' p refl₀ refl₀) ＝ ＝-cong idem₀ idem₀ refl₀
   use-eq₀ = eq₀' refl₀ p
 ...
 side₂-is-refl p = use-eq₂ ∙ (＝-cong-refl idem₂)
  where
   ...
   eq₂' : {a b c d : M}
        → (p : a ＝ b)
        → (q : c ＝ d)
        → (m' p p q) ＝ ＝-cong ((eq₂ a c) ⁻¹) ((eq₂ b d) ⁻¹) p
   eq₂' {a} {_} {c} {_} refl refl = (＝-cong-refl ((eq₂ a c) ⁻¹)) ⁻¹

   use-eq₂ : (m' refl₀ refl₀ p) ＝ ＝-cong idem₂ idem₂ refl₀
   use-eq₂ = eq₂' refl₀ p
...
\begin{code}
 ...
 M-is-set p = ((＝-cong-refl idem₁) ⁻¹) ∙ conjugate ∙ (side₁-is-p p)
  where
   conjugate : ＝-cong idem₁ idem₁ reflₘ ＝ ＝-cong idem₁ idem₁ (m' p refl₀ p)
   conjugate = (ap (＝-cong idem₁ idem₁) (almost-there p))
