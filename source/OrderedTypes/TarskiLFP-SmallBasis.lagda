...
and more importantly any x : L is generated by the basis, that is: x ＝ ⋁ ↓ᴮ x.
...
TarskiLFP is in someway a 'predicative taboo' but exactly how strong of a
predicative taboo is also open and a topic of current investigation.

Other modifications to the statement can be made in a similar vein. We can add
further restrictions to the sup-lattice and even the monotone map to acheive
a predicaitve proof of such a modified TarskiLFP (see
OrderedTypes.PredicativeLFP for details on a type theoretic formalization
inspired by Giovani Curi's work in the paper "On Tarski's Fixed Point Theorem").

But for now we put aside predicative concerns and will transition to the topic
of this file. Our goal is to show that TarskiLFP-small-basis 𝓤 𝓦 𝓥 is provable
in the presence of impredicative principles such as propositonal resizing.
...
author was stuck and moved onto other approaches. For example, using machinary
...
suggested that we consider closing the basis under the monotone map. For example

                          Σ (b : B) fⁿ (b) ???

Unfortunately, this approach still doesn't work because such a modified basis
would not necessarily be closed under infima; another critical fact used in the
traditional proof. But, this suggested an alternative approach to Carlo which
was subsequently sketched and explained to the author. We will formalize and
