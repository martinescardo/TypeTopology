...
open import DedekindReals.Reals.Properties
...
module DedekindReals.Reals.Addition
         (pe : Prop-Ext)
 ...
 where
  ...
  inhabited-left' = ∥∥-functor I (binary-choice (inhabited-from-real-L x) (inhabited-from-real-L y))
   where
    ...
    I ((k , k<x) , p , p<y) = p ℚ+ k , ∣ k , k<x , transport (_< y) (ℚ-inverse-intro'' fe p k) p<y ∣
  ...
  inhabited-right' = ∥∥-functor I (binary-choice (inhabited-from-real-R x) (inhabited-from-real-R y))
   where
    ...
    I ((k , x<k) , q , y<q) = (q ℚ- k) , ∣ k , x<k ,  transport (y <_) (ℚ-inverse-intro'''' fe q k) y<q  ∣
