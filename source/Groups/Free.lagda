...
universe 𝓤, the group freely generated by A is required to live in the
universe 𝓥, and the universal propertly eliminates into any universe 𝓦.
That the universal map η into the free group is an embedding and
small if A is a set is not part of the definition of freely generated
group, but we have it and we need it, and so we find it convenient to
include it in our definition of the existence of a "good" freely
generated group. The universe 𝓣 measures the size of the map η. Recall
that a map is said to be 𝓣-small if all its fibers are 𝓣-small.

\begin{code}

record good-freely-generated-group-exists (A : 𝓤 ̇ ) (𝓥 𝓣 : Universe) : 𝓤ω where
 field
  𝓕 : Group 𝓥
  η : A → ⟨ 𝓕 ⟩
  ...
  η-is-small     : is-set A → η is 𝓣 small-map
...
existence of small quotients in the presence of propositional
...
The third one drops set replacement, and instead constructs freely
generated groups, with ℓ = (_⁺), at the price of having the free group
living in the next universe (but still with the universal property
eliminating in all universes).
...
small, which is what the previous theorems give.
...
We can now prove the tecnical lemma.
...
be small, but it is needed for η to be an embedding.

We now proceed to the proof of the last theorem, which requires an
enhancement of the above proof.
...
The reduction relation _▷_ is defined in the module Group.Free,
...
will reduce it to 𝓤 in two steps.

\begin{code}

 η⁻-native-size : ηᴳʳᵖ is 𝓤⁺⁺ small-map
 η⁻-native-size = native-size-of-map ηᴳʳᵖ
...
The following remark is not used anywhere.
...
\begin{code}
  ...
  η⁻-is-small : η⁻ is 𝓤 small-map
  η⁻-is-small = ∘-small-maps ηᴳʳᵖ-is-small ⌜k⌝⁻¹-is-small
