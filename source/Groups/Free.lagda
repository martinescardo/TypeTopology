...
other than propositional truncation, and optionally quotients.
...
[1] above to make it "tiny".

We first formulate the main results of this file before proving them.

We start with a technical lemma, whose proof uses a general notion of
quotient, which allows the quotient of a given type to live in a
universe other than that of the given type. The parameter ℓ controls
...
reference [1], adapting the reasoning to HoTT/UF.
...
original relation has values on 𝓤⁺, and we get an equivalent relation
with values on 𝓤, exploiting the local smallness of A and introducing
 ...
 * Its enhancements are in the submodules resize-insertion-of-generators
...
for which we need to assume the existence of propositional
truncations.
...
existence of general, effective set quotients.
...
left-cancellable map, which relies on the effectivity of quotients.
...
Now notice that the natice size of ηᴳʳᵖ is large.
...
Using the results of the module resize-insertion-of-generators, we
