...
module positive-posets {𝓤  𝓦  𝓥 : Universe} (A : Poset 𝓤 𝓦) where
  ...
  pr₂ (strictly-below-implies-non-trivial x y i c) p =
    ...
    h : ∀[꞉]-syntax (upper-bound ((𝟙 + ((𝟘 , 𝟘-is-prop) holds)) , δ x y (𝟘 , 𝟘-is-prop)))
      (λ u → y ≤[ A ] (pr₁ u)) holds
    h u = y ＝⟨ p ⁻¹ ⟩ₚ pr₂ u (inl ⋆)

\end{code}

Ideally the more digestible syntax below would type check (ask someone with more experience) 

    h : ∀ u ꞉ (upper-bound ((𝟙 + ((𝟘 , 𝟘-is-prop) holds)) , δ x y (𝟘 , 𝟘-is-prop))) ,
      y ≤[ A ] (pr₁ u) holds
    h : ∀[꞉]-syntax (upper-bound ((𝟙 + ((𝟘 , 𝟘-is-prop) holds)) , δ x y (𝟘 , 𝟘-is-prop)))
      (λ u → y ≤[ A ] (pr₁ u)) holds

We also could show that if the converse holds then so does LEM in 𝓥.
...
\begin{code}
  ...
  pr₁ (transitivity-lemma x y z i) r = (≤-is-transitive A x y z (pr₁ r) (order (pr₂ r)) , h)
    ...
    h w q P l = sup-condition (pr₂ r) w q P ((a , b))
      ...
      b : ∀[꞉]-syntax (upper-bound ((𝟙 + (P holds)) , δ y w P)) (λ u → w ≤[ A ] pr₁ u ) holds 
      b u = pr₂ l ((pr₁ u , c))
        where
