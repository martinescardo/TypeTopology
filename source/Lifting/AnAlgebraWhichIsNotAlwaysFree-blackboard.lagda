...
\begin{code}
  ...
  h-at-l₀-explicitly : h l₀ ＝ ∀[꞉]-syntax (l₀ .pr₁) (λ a → ι (l₀ .pr₂ .pr₁ a))
...
corollary Ω∀-is-freely-generated-by-⊥
 = G-pointed-gives-excluded-middle ⋆
 where
  open main-results 𝟙 𝟙-is-set (λ ⋆ → ⊥Ω)
...
⊥-∀-positive-gives-EM ⊥-is-pos = III
 where
  ...
  II P P-is-prop ϕ = I P P-is-prop ϕ'
   where
    ϕ' : (P → 𝟘 {𝓣}) → 𝟘 {𝓣}
    ϕ' = λ (u : P → 𝟘 {𝓣}) → 𝟘-elim (ϕ (λ (a : P) → 𝟘-elim (u a)))
...
EM-gives-⊥-∀-positive em = III
 where
  ...
  II (P , P-is-prop) ϕ' = I P P-is-prop ϕ
   where
    ϕ : (P → 𝟘 {𝓤₀}) → 𝟘 {𝓤₀}
    ϕ = λ (u : P → 𝟘 {𝓤₀}) → 𝟘-elim (ϕ' (λ (a : P) → 𝟘-elim (u a)))
...
∀-positive-gives-equal-⊥ q@(Q , Q-is-prop) q-is-pos = II
 where
  ...
  I b = 𝟘-elim I₁
   where
    I₁ : 𝟘 {𝓣}
    I₁ = Ω∀-positivity-charac→ q q-is-pos ⊥Ω (λ (u : 𝟘 → Q) → b)
...
the-∀-positive-props-form-a-type-equiv-to-EM
 = logically-equivalent-props-are-equivalent
    ...
    (EM-is-prop fe')
    (λ (q , q-is-pos) → ⊥-∀-positive-gives-EM
                         (transport
                           (is-positive Ω∀)
                           (∀-positive-gives-equal-⊥ q q-is-pos)
                           q-is-pos))
    (λ em → (⊥Ω , EM-gives-⊥-∀-positive em))
