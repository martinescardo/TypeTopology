...
For Ω equipped with ∀ as its lifting structure, an element p : Ω is
positive, by definition, iff ((q → p) → p) → q for all q : Ω.

It follows from this that:

(i) ⊥ is positive iff the principle of excluded middle (EM) holds.

(ii) If p : Ω is positive then p ＝ ⊥.
...
TODO. Formalize these easy observations for the record.
