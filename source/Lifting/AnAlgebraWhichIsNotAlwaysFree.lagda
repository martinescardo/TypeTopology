...
Now we assume, for the sake of eventually concluding that the
principle of excluded middle holds, that Ω equipped with the structure
map ∀ is freely generated by G with insertion of generators ι.
...
know what ι is, namely the constantly ⊥ map, and so it is not really
arbitrary.

We now record that G isn't arbitrary either: we have that G ≃ 𝟙. For
this addendum, we invoke the results of our blackboard file.
...
\begin{code}
 ...
 G-is-𝟙 = qinveq
           ...
           ((λ ⋆ → ⌜ G-is-EM ⌝⁻¹ EM-holds) ,
            ...
            (λ (⋆ : 𝟙) → refl))
   where
    open Lifting.AnAlgebraWhichIsNotAlwaysFree-blackboard 𝓣 fe pe
