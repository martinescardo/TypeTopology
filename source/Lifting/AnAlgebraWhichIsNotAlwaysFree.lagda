...
As a corollary, we conclude that if Ω equipped with ⨆ = ∀ is freely
generated by ⊥, then the topos is boolean.

We take G ＝ 𝟙 and ι constantly ⊥. Then if the derived homomorphism
h : 𝓛 𝟙 → Ω is an equivalence, then the principle of excluded middle
holds.
...
module Ω∀-freely-generated-by-⊥-gives-EM where

 open main-results 𝟙 𝟙-is-set (λ _ → ⊥Ω)

 corollary : is-equiv h → EM 𝓣
 corollary h-is-equiv = G-pointed-gives-excluded-middle h-is-equiv ⋆
