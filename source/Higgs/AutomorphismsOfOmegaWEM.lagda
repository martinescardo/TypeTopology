...
Aut Ω has a sort of weak excluded middle: Every automorphism is either equal to
...
On the other hand, Aut Ω is sometimes a singleton. For another sheaf topos example, consider (the locale generated by) the total order (ℤ, ≤), every inhabited closed sublocale is equivalent to (ℤ, ≤) or (ℕ, ≤). Neither of these are boolean, so there are no nontrivial boolean closed sublocales of Ω in this topos.
