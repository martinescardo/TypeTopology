...
We work in a spartan Martin-Loef type theory, with the assumption that
...
injective. This is is not enough: for example, the unique map P → 𝟙 is
an embedding of P is a proposition, and the terminal type 𝟙 is small
but P doesn't need to be small (cf. work with Tom de Jong on size
matters).
...
Our free group is construced as a quotient of a set of words FA by a
...
FreeGroup, because here we resize down several of the constructions
performed in that file.
...
The reduction relation _▷_ is defined in the module FreeGroup, and its
propositional, symmetric, reflexive, transitive closure gives the
relation _∾_ that we use in order to quotient the type FA to get the
group freely generated by A.
...
We want that s ◗ t if and only the word t is reduct of s at some redex
r, which is what we prove next:
...
quotient FA to get the group freely generated by A.
...
the main theorem in this module:
...
The native size of the universal map ηη : FA → FA/∾ into the free
 ...
 ηη-is-medium = /-induction -∾- (λ y → fiber ηη y has-size 𝓤⁺)
  ...
  where
   ...
   γ s = (Σ a ꞉ A , η a ∥≏∥ s) , ≃-sym (d s)
    where
     notice : universe-of (fiber ηη (η/∾ s)) ≡ 𝓤⁺⁺
     notice = refl

\end{code}

But the above sizing of ηη is not small enough for our purposes.
...
With this we can further reduce the size of ηη to 𝓤:
...
\begin{code}
 ...
 ηη-is-small = /-induction -∾- (λ y → fiber ηη y has-size 𝓤)
  ...
  where
   ...
   d : (s : FA) → fiber ηη (η/∾ s) ≃ is-generator s
   d s = (Σ a ꞉ A , η/∾ (η a) ≡ η/∾ s) ≃⟨ Σ-cong (λ a → e a s) ⟩
...
Martin Escardo, recorded in the module UF-Size and recently submitted
for publication in a paper about size, says that if a map has size 𝓥,
and if also its codomain has size 𝓥, then so does its domain. Because
the type of ordinals is large by Burali-Forti, and because ηη has size
𝓤, as proved above, it follows that the free group must also be large,
which proves the promised second lemma:s

\begin{code}

 Lemma₂ : ¬ (⟨ free-group (Ordinal 𝓤) ⟩ has-size 𝓤)
 Lemma₂ h = the-type-of-ordinals-is-large (size-contravariance ηη ηη-is-small h)
...
anything to do with ordinals, and works with any locally small type A
: 𝓤⁺ so that we get its free group in the universe 𝓤⁺ rather than the
universe 𝓤⁺⁺ as in the module FreeGroup. In particular, Lemma₁ holds
for any locally small type A : 𝓤⁺.
