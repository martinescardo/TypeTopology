...
and prositional extensionality. This construction of quotients
increases the universal level by one, but eliminates into any
...
We work with a fixed, arbitrary universe 𝓤 in the remainder of this
file.
...
\begin{code}
 ...
 resized-ordinals-free-group-carrier : ⟨ free-group (Ordinal 𝓤) ⟩ has-size 𝓤⁺
 resized-ordinals-free-group-carrier = γ
  where
...
\begin{code}
 ...
 Lemma₁ = resized-group (free-group (Ordinal 𝓤)) resized-ordinals-free-group-carrier
...
size, says that if a map has size 𝓥, and if also its codomain has size
𝓥, then so does its domain. Because the type of ordinals is large by
Burali-Forti, and because ηᴳʳᵖ has size 𝓤, as proved above, it follows
that the free group must also be large, which proves the promised
second lemma:
