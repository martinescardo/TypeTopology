      ...
      continuous I α δ = u , v
       ...
       v u₁ p = transport (λ - → - ⊑⟨ 𝓕 ⟩ u₁)
         ...
         where
           ...
           _→e-is-directed = constant-function-is-directed 𝓔 (inhabited-if-directed (underlying-order 𝓓) α δ) e
           ...
           i→f⟨αi,e⟩-is-directed = image-is-directed (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (monotone-if-continuous (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (f , f-is-continuous)) i→⟨αi,e⟩-is-directed
           ...
           e₁ = f (∐ 𝓓 δ , e)
                 ...
                 f (∐ (𝓓 ×ᵈᶜᵖᵒ 𝓔) i→⟨αi,e⟩-is-directed)
                    ＝⟨ continuous-∐-＝ (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (f , f-is-continuous) i→⟨αi,e⟩-is-directed ⟩
        ...
        where
          ...
          v u₁ p = transport (λ - → - ⊑⟨ 𝓕 ⟩ u₁)
            ...
            where
              ...
              _→d-is-directed = constant-function-is-directed 𝓓 (inhabited-if-directed (underlying-order 𝓔) α δ) d
              ...
              i→f⟨d,αi⟩-is-directed = image-is-directed (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (monotone-if-continuous (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (f , f-is-continuous)) i→⟨d,αi⟩-is-directed
              ...
              e₁ = f (d , ∐ 𝓔 δ)
                    ...
                    f (∐ (𝓓 ×ᵈᶜᵖᵒ 𝓔) i→⟨d,αi⟩-is-directed)
                      ＝⟨ continuous-∐-＝ (𝓓 ×ᵈᶜᵖᵒ 𝓔) 𝓕 (f , f-is-continuous) i→⟨d,αi⟩-is-directed ⟩
  ...
  continuous-in-arguments→continuous f p₁ p₂ I α δ = u , v
    where
      ...
      f-is-monotone x y x₁ = transitivity 𝓕 (f x) (f (pr₁ x , pr₂ y)) (f y) ⊑₁ ⊑₂
        where
          ⊑₁ : f x ⊑⟨ 𝓕 ⟩ f (pr₁ x , pr₂ y)
          ⊑₁ = monotone-if-continuous 𝓔 𝓕 ((λ e → f (pr₁ x , e)) , p₂ (pr₁ x)) (pr₂ x) (pr₂ y) (pr₂ x₁)
          ⊑₂ : f (pr₁ x , pr₂ y) ⊑⟨ 𝓕 ⟩ f y
          ⊑₂ = monotone-if-continuous 𝓓 𝓕 ((λ d → f (d , pr₂ y)) , p₁ (pr₂ y)) (pr₁ x) (pr₁ y) (pr₁ x₁)
        ...
        where
          ...
          e₁ = transitivity 𝓕 ((f ∘ α) i) (f (pr₁ (α i) , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))) (f (∐ 𝓓 (pr₁∘α-is-Directed 𝓓 𝓔 δ) , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))) e₅ e₄
            where
              ...
              e₄ = monotone-if-continuous 𝓓 𝓕 ((λ x → f (x , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))) , (p₁ (∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ)))) (pr₁ (α i)) (∐ 𝓓 (pr₁∘α-is-Directed 𝓓 𝓔 δ)) (∐-is-upperbound 𝓓 (pr₁∘α-is-Directed 𝓓 𝓔 δ) i)
              e₅ : f (pr₁ (α i) , pr₂ (α i)) ⊑⟨ 𝓕 ⟩ f (pr₁ (α i) , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))
              e₅ = monotone-if-continuous 𝓔 𝓕 ((λ e → f (pr₁ (α i) , e)) , (p₂ (pr₁ (α i)))) (pr₂ (α i)) (∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ)) (∐-is-upperbound 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ) i)
      ...
      v u₁ p = transport (λ - → - ⊑⟨ 𝓕 ⟩ u₁) (e₁ ⁻¹) p₃
        where
          ...
          isdir₁ = image-is-directed 𝓓 𝓕 (monotone-if-continuous 𝓓 𝓕 ((λ - → f (- , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))) , p₁ (∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ)))) ((pr₁∘α-is-Directed 𝓓 𝓔 δ))
          f∘α-is-directed : is-Directed 𝓕 (f ∘ α)
          f∘α-is-directed = (inhabited-if-directed (underlying-order (𝓓 ×ᵈᶜᵖᵒ 𝓔)) α δ) , order
            where
              order : (i j : I) →
                        ∃
                        (λ k →
              ...
              order i j = ∥∥-functor (λ x → (pr₁ x) , ((f-is-monotone (α i) (α (pr₁ x)) (pr₁ (pr₂ x))) , (f-is-monotone (α j) (α (pr₁ x)) (pr₂ (pr₂ x))))) (semidirected-if-directed (underlying-order (𝓓 ×ᵈᶜᵖᵒ 𝓔)) α δ i j)
          e₁ : f (∐ (𝓓 ×ᵈᶜᵖᵒ 𝓔) δ) ＝ ∐ 𝓕 f∘α-is-directed
          e₁ = f (∐ (𝓓 ×ᵈᶜᵖᵒ 𝓔) δ)
                 ＝⟨ ap (λ - → f -) (∐⟨,⟩＝⟨∐,∐⟩ 𝓓 𝓔 δ) ⟩
               f (∐ 𝓓 (pr₁∘α-is-Directed 𝓓 𝓔 δ) , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))
                 ＝⟨ continuous-∐-＝ 𝓓 𝓕 ((λ d → f (d , ∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ))) , (p₁ (∐ 𝓔 (pr₂∘α-is-Directed 𝓓 𝓔 δ)))) (pr₁∘α-is-Directed 𝓓 𝓔 δ) ⟩
               ∐ 𝓕 isdir₁
                ...
                where
                  ...
                  u₂ i = pr₂ (p₂ (pr₁ (α i)) I (pr₂ ∘ α) (pr₂∘α-is-Directed 𝓓 𝓔 δ)) (∐ 𝓕 f∘α-is-directed) upper
                    where
                      ...
                      upper j = ∥∥-rec (prop-valuedness 𝓕 (f (pr₁ (α i) , pr₂ (α j))) (∐ 𝓕 f∘α-is-directed)) p₃ (semidirected-if-directed (underlying-order (𝓓 ×ᵈᶜᵖᵒ 𝓔)) α δ i j)
                        where
                          p₃ : Σ
                                 (λ k →
