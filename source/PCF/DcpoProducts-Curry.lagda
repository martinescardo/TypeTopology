    ...
    where
      ...
      f-is-continuous I α δ = u , v
        where
          ...
          u i e = sup-is-upperbound (underlying-order 𝓕) (continuity-of-function 𝓓 𝓕 a-fixed-e I α δ) i
            where
              a-fixed-e : DCPO[ 𝓓 , 𝓕 ]
          ...
          v u₁ p e = e₃ (underlying-function 𝓔 𝓕 u₁ e) (λ i → p i e)
            where
              ...
              e₃ =  sup-is-lowerbound-of-upperbounds (underlying-order 𝓕) (continuity-of-function 𝓓 𝓕 a-fixed-e I α δ)

  uncurryᵈᶜᵖᵒ : DCPO[ 𝓓 , 𝓔 ⟹ᵈᶜᵖᵒ 𝓕 ] → DCPO[ (𝓓 ×ᵈᶜᵖᵒ 𝓔) , 𝓕 ]
  uncurryᵈᶜᵖᵒ 𝕗@(f , f-is-continuous) = g , continuous-in-arguments→continuous 𝓓 𝓔 𝓕 g 𝓓→𝓕-is-continuous 𝓔→𝓕-is-continuous
    where
        ...
        where
          ...
          v u₁ p = transport (λ - → - ⊑⟨ 𝓕 ⟩ u₁) (ii ⁻¹) ∐-is-lowerbound
            where
              ...
              ⟨f∘α⟩i-is-directed = image-is-directed 𝓓 (𝓔 ⟹ᵈᶜᵖᵒ 𝓕) {f} f-is-monotone {I} {α} δ
  ...
  eval = f , c
    where
      ...
      f-is-monotone (g₁ , d₁) (g₂ , d₂) (g-⊑ , d-⊑) = f (g₁ , d₁)
                                                      ⊑⟨ 𝓔 ⟩[ monotone-if-continuous 𝓓 𝓔 g₁ d₁ d₂ d-⊑ ]
                                                      f (g₁ , d₂)
                                                      ⊑⟨ 𝓔 ⟩[ g-⊑ d₂ ]
                                                      f (g₂ , d₂) ∎⟨ 𝓔 ⟩
          ...
          continuous₂ g I α δ = u , v
            where
              ...
              v u₁ p = transport (λ - → - ⊑⟨ 𝓔 ⟩ u₁) (e₁ ⁻¹) p₁
                where
                  e₁ : f (g , ∐ 𝓓 δ) ＝  ∐ 𝓔 (image-is-directed 𝓓 𝓔 (monotone-if-continuous 𝓓 𝓔 g) δ)
                  e₁ = continuous-∐-＝ 𝓓 𝓔 g δ
                  p₁ : (∐ 𝓔 (image-is-directed 𝓓 𝓔 (monotone-if-continuous 𝓓 𝓔 g) δ)) ⊑⟨ 𝓔 ⟩ u₁
                  p₁ = ∐-is-lowerbound-of-upperbounds 𝓔 (image-is-directed 𝓓 𝓔 (monotone-if-continuous 𝓓 𝓔 g) δ) u₁ p
