...
Added by Fredrik Bakke on the 27th of March.

A type family Y is "uniformly decidable" if either every fiber has an element or
every fiber is empty. We show that decidable type families over decidable bases
with double negation dense equality are uniformly decidable, and hence that
their sums and products are decidable.

\begin{code}

is-uniformly-decidable-family : {X : 𝓤 ̇ } → (X → 𝓥 ̇ ) → 𝓤 ⊔ 𝓥 ̇
is-uniformly-decidable-family {𝓤} {𝓥} {X} Y =
 ((x : X) → Y x) + ((x : X) → ¬ (Y x))

decidable-families-over-bases-with-double-negation-dense-equality-are-uniformly-decidable : {X : 𝓤 ̇ }
                                                                                          → {Y : X
                                                                                               → 𝓥 ̇ }
                                                                                          → ((a : X)
                                                                                           → (b : X)
                                                                                          ...
                                                                                          → ((x : X)
                                                                                           → is-decidable (Y x))
                                                                                          → is-uniformly-decidable-family Y
decidable-families-over-bases-with-double-negation-dense-equality-are-uniformly-decidable {𝓤} {𝓥} {X} {Y} H dX dY = tada
 where
 tada : is-uniformly-decidable-family Y
 tada = cases positive-case negative-case dX
  where
   positive-case : X → is-uniformly-decidable-family Y
   ...
   negative-case : ¬ X → is-uniformly-decidable-family Y
...
uniformly-decidable-families-over-decidable-bases-give-decidable-Π : {X : 𝓤 ̇ }
                                                                   → {Y : X
                                                                        → 𝓥 ̇ }
                                                                   → is-decidable X
                                                                   → is-uniformly-decidable-family Y
                                                                   → is-decidable (Π Y)
uniformly-decidable-families-over-decidable-bases-give-decidable-Π {𝓤} {𝓥} {X} {Y} dX dY = tada
 where
...
decidable-families-over-decidable-bases-with-double-negation-dense-equality-give-decidable-Π : {X : 𝓤 ̇ }
                                                                                             → {Y : X
                                                                                                  → 𝓥 ̇ }
                                                                                             → ((a : X)
                                                                                              → (b : X)
                                                                                             ...
                                                                                             → ((x : X)
                                                                                              → is-decidable (Y x))
                                                                                             → is-decidable (Π Y)
decidable-families-over-decidable-bases-with-double-negation-dense-equality-give-decidable-Π {𝓤} {𝓥} {X} {Y} H dX dY = tada
 where
 tada : is-decidable (Π Y)
 tada = uniformly-decidable-families-over-decidable-bases-give-decidable-Π dX ø
  where
  ø : is-uniformly-decidable-family Y
  ø = decidable-families-over-bases-with-double-negation-dense-equality-are-uniformly-decidable H dX dY
...
uniformly-decidable-families-over-decidable-bases-give-decidable-Σ : {X : 𝓤 ̇ }
                                                                   → {Y : X
                                                                        → 𝓥 ̇ }
                                                                   → is-decidable X
                                                                   → is-uniformly-decidable-family Y
                                                                   → is-decidable (Σ Y)
uniformly-decidable-families-over-decidable-bases-give-decidable-Σ {𝓤} {𝓥} {X} {Y} dX dY = tada
 where
...
decidable-families-over-decidable-bases-with-double-negation-dense-equality-give-decidable-Σ : {X : 𝓤 ̇ }
                                                                                             → {Y : X
                                                                                                  → 𝓥 ̇ }
                                                                                             → ((a : X)
                                                                                              → (b : X)
                                                                                             ...
                                                                                             → ((x : X)
                                                                                              → is-decidable (Y x))
                                                                                             → is-decidable (Σ Y)
decidable-families-over-decidable-bases-with-double-negation-dense-equality-give-decidable-Σ {𝓤} {𝓥} {X} {Y} H dX dY = tada
 where
 tada : is-decidable (Σ Y)
 tada = uniformly-decidable-families-over-decidable-bases-give-decidable-Σ dX ø
  where
  ø : is-uniformly-decidable-family Y
  ø = decidable-families-over-bases-with-double-negation-dense-equality-are-uniformly-decidable H dX dY
