...
construction of quotients increases the universe level by one (but it
...
In this file with work with a type A : 𝓤⁺ and we show that the free
group constructed in the module FreeGroup.lagda, which lives in the
universe 𝓤⁺⁺, has a copy in the same universe 𝓤⁺ where A lives,
provided A is locally small (its identity types, which live in 𝓤⁺,
have equivalent copies in 𝓤). Moreover, we show that if the group
freely generated by A has a copy in the universe 𝓤, then A itself must
have a copy in 𝓤.  We then apply this in the module BuraliForti.lagda
with A the type of ordinals in the universe 𝓤, which doesn't have a
copy in 𝓤, from which we conclude that the free group also doesn't
have a copy in 𝓤.
...
map P → 𝟙 is an embedding if P is a proposition, and the terminal type
𝟙 is of course small, but P doesn't need to be small - cf. work with
Tom de Jong on size matters https://arxiv.org/abs/2102.08812, from
which we borrow other techniques in the development below.
...
universe 𝓥, and that a map has size 𝓥 if its fibers all have size
𝓥. See the module UF-Size.lagda.
...
It follows that if there is a large, locally small set, then then
...
In the module BuraliForti.lagda we instantiate this to A the type of
